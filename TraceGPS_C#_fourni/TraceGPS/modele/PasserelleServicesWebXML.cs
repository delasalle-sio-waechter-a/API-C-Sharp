...
namespace TraceGPS
{
    public class PasserelleServicesWebXML : PasserelleXML
    {
        ...
        private static String _adresseHebergeur = "http://localhost/ws-php-waechter.a/tracegps/api/";
        ...
        public static String getLesUtilisateursQueJautorise(String pseudo, String mdpSha1, ArrayList lesUtilisateurs)
        {
            ...
            {	// création d'un nouveau document XML à partir de l'URL du service web et des paramètres
                ...
                urlDuServiceWeb += "?pseudo=" + pseudo;
                urlDuServiceWeb += "&mdp=" + mdpSha1;
                urlDuServiceWeb += "&lesUtilisateurs=" + lesUtilisateurs;
...
//            return "";          // code provisoire, méthode à écrire et à tester !!!
        ...
        public static String getLesParcoursDunUtilisateur(String pseudo, String mdpSha1, String pseudoConsulte, ArrayList lesTraces)
        {
            ...
            {	// création d'un nouveau document XML à partir de l'URL du service web et des paramètres
                ...
                leDocument.ReadToFollowing("donnees");
                leDocument.ReadToFollowing("lesTraces");
                // parcours de la liste des noeuds <utilisateur> et ajout dans la collection lesUtilisateurs
                while (leDocument.ReadToFollowing("trace"))
                {
                    ...
                    DateTime uneDateHeureDebut = Convert.ToDateTime(leDocument.Value.ToString());

                    leDocument.ReadToFollowing("terminee"); leDocument.Read();
                    bool terminee;

                    if (leDocument.Value == "1")
                    { terminee = true; }
                    else
                    { terminee = false; };

                    if (Convert.ToInt32(leDocument.Value) == 1)
                    {
                        leDocument.ReadToFollowing("dateHeureFin"); leDocument.Read();
                        DateTime uneDateHeureFin = Convert.ToDateTime(leDocument.Value.ToString());
                    }

                    leDocument.ReadToFollowing("distance"); leDocument.Read();
                    double uneDistance = Convert.ToDouble(leDocument.Value.Replace(".",","));
                    ...
                    if (Convert.ToInt32(leDocument.Value) == 1)
                    {
                        ...
                        DateTime uneDateHeureFin = DateTime.MinValue;
                        Trace uneTrace = new Trace(unId, uneDateHeureDebut, uneDateHeureFin, terminee, unIdUtilisateur, uneDistance);
                        lesTraces.Add(uneTrace);
                    }
        ...
        public static String getUnParcoursEtSesPoints(String pseudo, String mdpSha1, int idTrace, Trace laTrace)
        {
            ...
            {	// création d'un nouveau document XML à partir de l'URL du service web et des paramètres
                ...
                while (leDocument.ReadToFollowing("trace"))
                {
                    ...
                    leDocument.ReadToFollowing("lesPoints");

                    while (leDocument.ReadToFollowing("point"))
                    {



                        leDocument.ReadToFollowing("id"); leDocument.Read();
                        int id = Convert.ToInt32(leDocument.Value);

                        leDocument.ReadToFollowing("latitude"); leDocument.Read();
                        double uneLatitude = Convert.ToDouble(leDocument.Value.Replace(".", ","));

                        leDocument.ReadToFollowing("longitude"); leDocument.Read();
                        double uneLongitude = Convert.ToDouble(leDocument.Value.Replace(".", ","));

                        leDocument.ReadToFollowing("altitude"); leDocument.Read();
                        double uneAltitude = Convert.ToDouble(leDocument.Value.Replace(".", ","));

                        leDocument.ReadToFollowing("dateHeure"); leDocument.Read();
                        DateTime uneDateHeure = Convert.ToDateTime(leDocument.Value.ToString());

                        leDocument.ReadToFollowing("rythmeCardio"); leDocument.Read();
                        int unRythmeCardio = Convert.ToInt32(leDocument.Value);


                        PointDeTrace unPoint = new PointDeTrace(idTrace,id,uneLatitude,uneLongitude,uneAltitude,uneDateHeure,unRythmeCardio);
